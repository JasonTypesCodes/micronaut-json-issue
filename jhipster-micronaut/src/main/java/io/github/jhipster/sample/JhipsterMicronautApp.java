...
public class JhipsterMicronautApp {

    private static final Logger log = LoggerFactory.getLogger(JhipsterMicronautApp.class);

    /**
     * Initializes JhipsterMicronautApp.
     * <p>
    ...
    public static void main(String[] args) {
        ...
        Map<String, Object> options = commandLine.getUndeclaredOptions();
        List<String> environments;
        if (options.containsKey("profiles.active")) {
            String[] profiles = options.get("profiles.active").toString().split(",");
            environments = Arrays.asList(profiles);
        } else {
            environments = DefaultProfileUtil.getDefaultEnvironments();
        }

        ApplicationContext context = Micronaut.build(args)
            .mainClass(JhipsterMicronautApp.class)
